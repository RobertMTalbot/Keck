...
The parallel analysis suggest 3 components and 6 factors and the scree plot shows 3 components with eigenvalue > 1, so run PCA (descriptive model) with 3 factors, varimax rotation

```{r}
principal(X1516GCA_FA, nfactors=3, rotate = "varimax")
```

Now compare to a factor analysis (structural model) specifying 3 factors, varimax rotation, do not impute values for missing, use minimum residual factoring method (default) and view loading matrix
