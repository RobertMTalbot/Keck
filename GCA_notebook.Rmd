...
Now filter (using dplyr) All.data for cl = 1, then find pre and post GCA mean, SD, %NA (using psych describe). Then calculate gains two ways: once with na.rm = TRUE and once with na.rm =  FALSE
```{r}
library(dplyr)
library(psych)
...
I need to make the above code a function and then use lapply(?) to iterate over all classes, or just increment and loop it over and over. Then write each resulting object (e.g. cl1.descr, cl1_g1, etc) to a new table. Thoughts on how to code that nicely?

##Local item <g> and ES
Next we need to calculate pre and post totals and gains (<g>) for the local items for each class (note that this will not be for Fa15 or Sp16, only Fa16 and Sp17). 
